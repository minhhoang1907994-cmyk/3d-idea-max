...
import type { ConvertedValue, MachineProcessPreset, ProjectInspection } from '../types';
  ...
  for (const { mapping, value } of inspection.settings) {
    if (mapping.profile === 'filament') {
      byKey[mapping.orcaKey] = {
        kind: 'unavailable',
        reason: 'Phụ thuộc cuộn filament bạn nạp, không phải máy — lấy theo nhãn cuộn nhựa.',
      };
      continue;
    }
