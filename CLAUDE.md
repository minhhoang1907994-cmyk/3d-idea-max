...
| 4   | Hỗ trợ 2 máy: **Bambu Lab A1, Anycubic Kobra X** (mặc định) | Thông số + giới hạn khổ in đổi theo máy. Trước có thêm A1 mini / P1S / X1C — đã gỡ vì nhóm không dùng; thông số vẫn nằm trong `docs/research/` nếu cần thêm lại                                                                                                                                                                                                                                                                                                                                    |
...
- **Key Backblaze thì ngược lại — nằm trong bundle.** Ảnh của Sổ công ty tải thẳng lên
  B2 từ trình duyệt, key lấy từ `VITE_B2_*` lúc build nên ai xem source cũng đọc được.
  Chấp nhận được vì key bị giới hạn đúng một bucket, giống cách role `app_editor` giới
  hạn quyền trên Neon. Khác Gemini key ở chỗ cả nhóm dùng chung một kho ảnh, bắt mỗi
  người tự nhập key thì không ai dùng. Xem `docs/backblaze-setup.md`
